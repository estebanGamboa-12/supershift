...
export default function CalendarView({
  ...
  return (
    <div className={containerClassName}>
      ...
      {/* Weekdays */}
      <div className="grid grid-cols-7 gap-px border-b border-white/10 bg-slate-900/60 text-[12px] font-semibold uppercase tracking-wide text-white/60 sm:text-xs">
        {weekdays.map((day) => (
          ...
          {calendarDays.map((day) => {
            ...
            return (
              <div
                ...
                className={`group flex flex-col gap-2 rounded-xl border border-white/5 bg-slate-950/40 p-3 text-sm transition hover:border-blue-400/40 hover:bg-slate-900/70 sm:p-2 sm:text-xs ${
                  isCurrentMonth ? "text-white/90" : "text-white/40"
                }`}
                style={{ minHeight: "70px" }} // 👈 más alto en móvil
              >
                {/* Número del día */}
                <div className="flex items-center justify-between">
                  <span
                    className={`flex h-7 w-7 items-center justify-center rounded-full text-sm font-semibold transition sm:h-6 sm:w-6 sm:text-xs ${
                      isCurrentDay
